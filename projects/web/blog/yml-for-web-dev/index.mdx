...
subtitle: Um guia simples para ajudar você a ler e escrever arquivos .yml
date: '2020-03-19'
...
De acordo com o [site oficial](https://yaml.org/), Yaml é (tradução livre):

<BigQuote>
  "YAML (um acrónimo recursivo para "YAML Ain't Markup Language" [algo como, não
  é uma linguagem de marcação]) é um padrão amigável para serialização de dados
  para todas as linguagens de programação."
</BigQuote>
...
> "Superset é uma linguagem de programação que contém todas as funcionalidades de uma outra, porém expandida ou com mais funcionalidades." - [Fonte](https://encyclopedia2.thefreedictionary.com/superset)
...
Em Yaml, indentação **é importante**. Ele usa _espaços em branco_ para agrupar informações. Por espaço em branco, entenda apenas como literalmente espaço (space), uma vez que o Tab não é permitido.

> Não se assuste. Se você, assim como eu, usa o tab para indentar código, um simples arquivo [`.editorconfig`](<(https://editrconfig.org/)>), uma configuração ou um plugin no editor resolvem isso facilmente. Assim, quando você apertar tab, o editor troca o caractere de Tab por barras de espaços (2 ou 4).
...
> Não acredita? Copia e cola o código Yaml anterior [nesta ferramenta](https://onlineyamltools.com/convert-yaml-to-json) e veja com seus próprios olhos.
...
- [JSON ←→ YAML Online converter](https://www.json2yaml.com/);
